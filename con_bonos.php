...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_bono='2'";
	$error=editar("bono",$campos,'cod_bono',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_bono = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_bono = 1";
}

$sql="SELECT * FROM bono
INNER JOIN tipo_bono ON tipo_bono.cod_tbono = bono.tipo_bono $where";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_d_bono.php">
                  <table width="90%" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td><table width="90%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php  if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_d_bono.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php  } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="num_bono">NUMERO</option>
                            <option value="nom_tbono">TIPO</option>
                            <option value="valor_bono">VALOR</option>
                    ...
                    <tr>
                      <td><table width="90%" border="0"  cellpadding="0">
						...
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_d_bono.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_bono' action='man_d_bono.php'>  ";
              echo "<td >$db->num_bono</td>";
						  echo "<td  >$db->nom_tbono</td> ";
              echo "<td  >$db->valor_bono</td> ";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_bono'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_bono.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_bono) /></td> ";
							else
...
<form name="forma" method="post" action="con_bonos.php">
