...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
	$campos="estado_tmov='2'";
	$error=editar("tipo_movimientos",$campos,'cod_tmov',$eli_codigo); 
	if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
	...
	$where=" where estado_tmov = 1 and $busquedas";
}#codigo para buscar 
else{
	$where=" where estado_tmov = 1";
}

$sql="SELECT * FROM tipo_movimientos $where";
                    ...
                    <tr>
                      <td><table width="90%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
						  <?php  if ($insertar==1) {?>
					  	  <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_consecutivo.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php  } ?></td>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_tmov">Nombre</option>
                            <option value="desc_tmov">Descripcion</option>
                            <option value="num_cons">Consecutivo</option>
                    ...
                    <tr>
                      <td><table width="90%" border="0"  cellpadding="0">
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_tmov' action='man_consecutivo.php'>  ";
                          	echo "<td >$db->nom_tmov </td>";
                            echo "<td >$db->desc_tmov </td>";
                            echo "<td >$db->num_cons </td>";
                            echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_tmov'>";
							if ($editar==1)
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_tmov.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==1)
								echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_tmov) /></td> ";
							else
