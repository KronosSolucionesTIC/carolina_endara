...
if($_REQUEST['eliminacion']==1) {//confirmacion de insercion  
  $campos="estado_color='2'";
  $error=editar("color",$campos,'cod_color',$eli_codigo); 
  if ($error >=1)
...
if(!empty($busquedas)) { #codigo para buscar 
  ...
  $where=" where estado_color = 1 and $busquedas";
}#codigo para buscar 
else{
  $where=" where estado_color = 1";
}

$sql="SELECT * FROM color $where";
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_color.php">
                    ...
                    <tr>
                      <td><table width="90%" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td> 
              <?php  if ($insertar==1) {?>
                <img src="imagenes/agregar.png"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_color.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
                <?php  } ?></td>
              ...
              <?php  } ?>
                          ...
                          <td valign="middle"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="nom_color">Nombres</option>
            ...
            echo "<tr style='display:none'><td ><form name='forma_0' action='man_color.php'>";
            echo "  </form> </td></tr>  ";              
            $estilo="ctablablanc";
            $estilo="ctablagris";
            ...
            while($db->next_row()) {
              ...
              echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_color' action='man_color.php'>  ";
                            echo "<td >$db->nom_color </td>";
                            echo "<td aling='center' >"; 
              echo  "<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo  " <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_color'>";
              if ($editar==1)
                echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' style='cursor:pointer'  onclick='document.forma_$db->cod_color.submit()'/></td>";
              else 
                echo "<img src='imagenes/e_icoeditar.png'   /></td>";
                            echo  "<td align='center'>";
              if ($eliminar==1)
                echo"<img src='imagenes/icoeliminar.png'  alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_color) /></td> ";
              else
...
<form name="forma" method="post" action="con_color.php">
