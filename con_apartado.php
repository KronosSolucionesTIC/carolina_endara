...
if($det==0)
  $where.=" where cod_apa>0   and  ( $where_cli )  ";
...
left join usuario on usuario.cod_usu=m_apartado.cod_usu
left join cliente on cliente.cod_cli=m_apartado.cod_cli 
left join bodega on bodega.cod_bod =m_apartado.cod_bod $where ORDER BY cod_apa  DESC ";
//exit;
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_apartado.php">
                    ...
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="17"> 
              <?php if ($insertar==1) {?>
                <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_d_apartado.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
                <?php } ?></td>
              ...
              <td  class="ctablasup">APARTADO</td>
                        </tr>
            <?php 
            
            echo "<tr style='display:none'><td ><form name='forma_0' action='man_apartado.php'>";
            echo "  </form> </td></tr>  ";              
            $estilo="ctablablanc";
            $estilo="ctablagris";
            ...
            while($db->next_row()) {
              ...
              echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_apa' action='man_apartado.php'>  ";
              echo "<td >$db->fecha </td>";
...
$vend = $dbv->nom_ven;
              ...
              $dbr = new database();
              $sqlr = "SELECT * FROM rsocial WHERE cod_rso = $db->cod_razon_apa";
              $dbr->query($sqlr);
              $dbr->next_row();
...
<form name="forma" method="post" action="con_apartado.php">
