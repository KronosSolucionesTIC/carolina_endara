...
function cambio_guardar() {
    ...
    if (mensaje == true) {
        ...
        document.getElementById('docdiv_boton_guardar').innerHTML = '';
        document.forma.submit();
    } else alert('Complete el Formulario, Gracias')
}
...
function validaFloat(obj) {
    if (event.keyCode == 46 && obj.value == "") obj.value = "0";
    ...
    if (event.keyCode > 47 & event.keyCode < 58 || event.keyCode == 46) return true;
    else return false;
}

function validaValue(obj) {
    var arreglo = obj.value.split(".");
    if (obj.value.substring(0, 1) == ".") obj.value = "0" + obj.value;
    if (parseInt(arreglo[1]) == 0) obj.value = parseInt(arreglo[0]);
    if (obj.value.substring(obj.value.length - 1, obj.value.length) == ".") obj.value = obj.value.substring(0, obj.value.length - 1);
    ...
    if (document.getElementById(boton).value == 'Marcar') {
        ...
        for (i = 1; i <= final; i++) {
            if (document.getElementById(obj + i)) {
                document.getElementById(obj + i).checked = 1;
            }
        }
    } else {
        ...
        for (i = 1; i <= final; i++)
            if (document.getElementById(obj + i)) {
                document.getElementById(obj + i).checked = 0;
            }
    }
}
