...
if($eliminacion==1) {//confirmacion de insercion  
	$error=eliminar("bono",$eli_codigo,"cod_bono");
...
if(!empty($busquedas)) { #codigo para buscar 
 ...
 $sql="select * , CONCAT(cliente.nom1_cli,apel1_cli) as nom_cliente  from bono 
left join cliente on cliente.cod_cli=bono.cli_bono
INNER JOIN vendedor ON vendedor.cod_ven = bono.ven_bono
$where AND estado_bono = 1 
ORDER BY cod_bono  DESC ";
//exit;
...
<tr>
<td valign="top" >
<form id="forma_total" name="forma_total" method="post" action="man_anul_bono.php">
                  <table width="644" border="0" cellspacing="0" cellpadding="0" align="center" >
                    <tr>
                      <td bgcolor="#D1D8DE"><table width="624" border="0" cellspacing="0" cellpadding="0">
                        <tr>
                          ...
                          <td width="17"> 
						  <?php if ($insertar==10) {?>
					  	  <img src="imagenes/agregar.png" width="16" height="16"  alt="Nuevo Registro" style="cursor:pointer" onClick="location.href='man_anul_bono.php?codigo=0&editar=<?php echo $editar?>&insertar=<?php echo $insertar?>&eliminar=<?php echo $eliminar?>'"/>
					  	  <?php } ?></td>
                          ...
                          <td width="160" class="ctablaform"><select name="campos" class="textfieldlista" id="campos" >
                            ...
                            <option value="fec_bono">Fecha</option>
                            <option value="num_bono">No bono</option>
                    ...
                    <tr>
                      <td><table width="638" border="0"  cellpadding="0">
						...
						<?php 
						echo "<tr style='display:none'><td ><form name='forma_0' action='man_anul_bono.php'>";
						echo "  </form> </td></tr>  ";						  
						$estilo="ctablablanc";
						$estilo="ctablagris";
						//echo $sql;
						...
						while($db->next_row()) {
							...
							echo "<tr class='$estilo' $cambio_celda> <form name='forma_$db->cod_bono' action='man_anul_bono.php'>  ";
							echo "<td >$db->estado</td>";
							echo "<td >$db->fec_bono</td>";
							echo "<td >$db->num_bono</td>";
							echo "<td >$db->nom_ven</td>";
							echo "<td >$db->nom_cliente </td>";
							echo "<td aling='center' >"; 
							echo 	"<table width='100%' border='0' cellspacing='0' cellpadding='0'>";
                            echo 	" <tr>  <td align='center'> <input type='hidden' name='codigo' value='$db->cod_bono'>";
							if ($editar==1 && $db->estado!='anulado' )
							 	echo "<img src='imagenes/icoeditar.png' alt='Editar Registro' width='16' height='16' style='cursor:pointer'  onclick='document.forma_$db->cod_bono.submit()'/></td>";
							else 
								echo "<img src='imagenes/e_icoeditar.png' width='16' height='16'  /></td>";
                            echo 	"<td align='center'>";
							if ($eliminar==10)
								echo"<img src='imagenes/icoeliminar.png' width='16' alt='Eliminar Registro' height='16' style='cursor:pointer' onclick=confirmar($db->cod_bono) /></td> ";
							
                           
						   //impresion	
							echo "<td align='center'><img src='imagenes/mirar.png' width='16' height='16'  style=\"cursor:pointer\" onClick=\"imprimir_inf('ver_bono.php',$db->cod_bono,'grande')\" /></td>";	
...
<form name="forma" method="post" action="con_anul_bono.php">
