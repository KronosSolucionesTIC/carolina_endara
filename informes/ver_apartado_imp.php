...
include("../conf/clave.php");				
	$db = new Database();
	$db_ver = new Database();
	$sql = "select *, DATE_ADD(fecha ,interval 30 day) as fac_fecha_vence   from m_apartado where cod_apa=$codigo";
	$db_ver->query($sql);	
	if($db_ver->next_row())
	{ 
		$rem_apa=$db_ver->rem_apa;
		$cod_razon=$db_ver->cod_razon_apa;
		$fac_numero=$db_ver->num_apa;
		$fac_fecha=$db_ver->fecha;
		$tipo_apa=$db_ver->tipo_apa;
		$codigo_bod=$db_ver->bod_cli_apa;
		$codigo_cli=$db_ver->cod_cli;
		$codigo_razon=$db_ver->cod_razon_apa;
		$tipo_pago=$db_ver->tipo_pago;
		$fac_fecha_vence=$db_ver->fac_fecha_vence;
		$estado_factura=$db_ver->estado;
		$cod_usuario=$db_ver->cod_usu;
		$obs_apa =$db_ver->obs;
		$tot_apa = $db_ver->tot_apa;
	...
	<TR>
			...
			<TR>
			  <TD><table width="100%" align="center" border="0"  id="select_tablas">
				...
				left join tipo_producto on d_apartado.cod_tpro=tipo_producto.cod_tpro
				left join marca on d_apartado.cod_cat=marca.cod_mar
				left join talla on talla.cod_talla = d_apartado.cod_talla
				left join color on color.cod_color = d_apartado.cod_color
				left join producto  on d_apartado.cod_pro= producto.cod_pro 
				...
				$total++;
				  ...
				  $sql = "SELECT tot_apa as total FROM m_apartado WHERE cod_apa= $codigo";
			$db->query($sql);
			if($db->next_row()){ 
				$total = $db->total;
			}	
				$saldo = $tot - $tot_apa ; 
				$descuento = $total - $tot_apa;
				?>
                <tr>
                  <td colspan="5">&nbsp;</td>
                </tr>
				...
				INNER JOIN m_apartado ON m_apartado.bono_apa = bono.cod_bono
				WHERE cod_apa = $codigo";
				$dbb1->query($sqlb1);
				if($dbb1->next_row()){
				?>
				<tr>
                ...
                <tr>
                  ...
                  <td class="textotabla01"><div align="right" class="Estilo3"><?PHP echo '$'.number_format($tot_apa,0,".",".")?></span></div></td>
                </tr>
               	<tr>
                  <td class="textotabla01"></td>
                  <td class="textotabla01"></td>
				...
				INNER JOIN d_apartado ON d_apartado.cod_pro = producto.cod_pro
			...
			$sql = "SELECT SUM(total_pro) as total FROM d_apartado WHERE cod_mapa= $codigo";
			$db->query($sql);
			if($db->next_row()){ 
				$total = $db->total;
			}	
			...
			<table width="100%" border="0" align="center">
              <tr>
                <td width="100%" align="center"><div align="center" class="Estilo3">PRENDAS APARTADAS NO TIENEN CAMBIO, SE GUARDA POR UN MES, NO SE HACE DEVOLUCION DE DINERO,NO SE HACE DEVOLUCION DE DINERO,NO SE HACE NING&Uacute;N ARREGLO HASTA TANTO LA PRENDA EST&Eacute; CANCELADA EN SU TOTALIDAD
     			</div>
