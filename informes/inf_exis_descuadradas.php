...
$db_ver->cod_pro = $_REQUEST['producto'];
...
function abrir(val){
	alert(val);
	imprimir_inf("../inf_det_movimiento_referencia.php",'0&producto='+val,'mediano','target = _blank');	
}
</script>
...
<TABLE width="100%" border="0" cellspacing="0" cellpadding="0"   >
	...
	<TR>
		...
		<TABLE width="98%" border="1" cellpadding="0" cellspacing="0" bordercolor="#999999" >
			...
			<INPUT type="hidden" name="producto" id='producto' value="<?php echo $db_ver->cod_pro?>">
			...
			<TR>
			  ...
			  <table width="100%" border="1" cellpadding="1" cellspacing="0" bordercolor="#333333">
			  	<tr>
			  		<td><div align="center">INFORME DE EXISTENCIAS DESCUADRADAS</div></td>
			    </tr>
				</table>					</TD>
		  </TR>
			...
			<TR>
			  ...
			  <table width="100%" border="1" cellpadding="1" cellspacing="1" bordercolor="#333333" id="select_tablas" >
			    <tr >
			      <td  class="botones1"><div align="center">COD</div></td>
			...
			while($db_ver->next_row()){
				...
				WHERE cod_ref_dent = $db_ver->cod_pro AND cod_bod = 1 AND estado_m_entrada = 1";
				$dbi->query($sqli);
				while($dbi->next_row()){
				...
				WHERE estado is null AND cod_pro = $db_ver->cod_pro";
				$dbv->query($sqlv);
				while($dbv->next_row()){
				...
				WHERE cod_ref_dtra = $db_ver->cod_pro AND cod_bod_ent_tras = 1";
				$dbt->query($sqlt);
				while($dbt->next_row()){
				...
				WHERE cod_ref_kar = $db_ver->cod_pro AND cod_bod_kar = 1
				...
				//CALCULA CUALES ESTAN DESCUADRADOS
				$suma1 = $ingresos - $traslados - $ventas;
				$suma2 = $existencias;

				if($suma1 != $suma2){											
				?>
                <tr id="fila_0">
                	<td class="textotabla01"><div align="center"><?php echo $db_ver->cod_pro ?></div></td>			
					...
					<td class="textotabla01"><div align="center"><img src='../imagenes/mirar.png' width='16' height='16'  style="cursor:pointer"  onclick="abrir(<?php echo $db_ver->cod_pro ?>)" /></div></td>
			    </tr>
			    <?php } ?>
			<?php } ?>				 
				<tr>			  
